...
class ValidatorTest(unittest.TestCase):
    ...
    def test_token_efficient_prompt_colocates_final_planning_and_validation(self):
        ...
        self.assertIn("immutable canonical value", prompt)
        self.assertIn("Copy it byte-for-byte into every Workflow input", prompt)
        self.assertIn("never append or prepend inferred metadata", prompt)
        ...
        self.assertIn("Do not use `agent_config` in this trial", prompt)
        self.assertIn("a one-argument model shortcut is invalid", prompt)
